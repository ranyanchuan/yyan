(function($, window) {
	...
	var template = '<div id="{{id}}" class="yyan-slider yyan-preview-image yyan-fullscreen"><div class="yyan-preview-header">{{header}}</div><div class="yyan-slider-group"></div><div class="yyan-preview-footer yyan-hidden">{{footer}}</div><div class="yyan-preview-loading"><span class="yyan-spinner yyan-spinner-white"></span></div></div>';
	var itemTemplate = '<div class="yyan-slider-item yyan-zoom-wrapper {{className}}"><div class="yyan-zoom-scroller"><img src="{{src}}" data-preview-lazyload="{{lazyload}}" style="{{style}}" class="yyan-zoom"></div></div>';
	var defaultGroupName = '__DEFAULT';
	var div = document.createElement('div');
	var imgId = 0;
	var PreviewImage = function(options) {
		this.options = $.extend(true, {
			id: '__yyan_PREVIEWIMAGE',
			zoom: true,
			header: '<span class="yyan-preview-indicator"></span>',
			footer: ''
		}, options || {});
		this.init();
		this.initEvent();
	...
	proto.initEvent = function() {
		...
		this.element.addEventListener('slide', function(e) {
			if (self.options.zoom) {
				var lastZoomerEl = self.element.querySelector('.yyan-zoom-wrapper:nth-child(' + (self.lastIndex + 1) + ')');
				if (lastZoomerEl) {
	...
	proto.addImage = function(img) {
		...
		if (img.__yyan_img_data && img.__yyan_img_data.src === src) { //已缓存且图片未变化
			this.groups[group].push(img.__yyan_img_data);
		} else {
	...
	proto.open = function(index, group) {
		...
		if (typeof index === "number") {
			...
			group = group || defaultGroupName;
			this.addImages(group, index); //刷新当前group
			this.openByGroup(this.groups[group].indexOf(index.__yyan_img_data), group);
		}
	};
